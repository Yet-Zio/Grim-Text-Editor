...
# MenuBar-Help menu
    ...
    def OnOpen(self, e):
        try:
            ...
            dlg = wx.MessageDialog(self, "Couldn't open file!\nWere you trying to open a machine coded file?", "Error: Soul not recognised", wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
    ...
    def OnSave(self, e):
        try:
            ...
            try:
                dlg = wx.FileDialog(self, "Save file as", self.dirname, "New Soul Document.soul", "*.*", wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT)
                if (dlg.ShowModal() == wx.ID_OK):
                    self.filename = dlg.GetFilename()
                    self.dirname = dlg.GetDirectory()
                    f = open(os.path.join(self.dirname, self.filename), 'w')
